package co.esteban.anotaciones.ejemplo.procesador;

import co.esteban.anotaciones.ejemplo.Init;
import co.esteban.anotaciones.ejemplo.JsonAtributo;
import co.esteban.anotaciones.ejemplo.procesador.exception.JsonSerializadorException;
        ...
        return Arrays.stream(atributos).filter(f -> f.isAnnotationPresent(JsonAtributo.class))
                .map(f -> {
                    ...
                    String nombre = f.getAnnotation(JsonAtributo.class)
                            .nombre().equals("") ? f.getName() : f.getAnnotation(JsonAtributo.class).nombre();
